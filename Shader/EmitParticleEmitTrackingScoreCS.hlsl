...
void main(uint3 DTid : SV_DispatchThreadID)
{
    ...
    if (DTid.x < emitCount)
    {
        ...
        particle.maxLife = particleParam[constantIndex].xParticleLifeSpan;
        particle.life = particle.maxLife;
        particle.sizeBeginEnd = float2(particleStartingSize + particleParam[constantIndex].xParticleScaling, particleStartingSize);
        particle.color = 0;

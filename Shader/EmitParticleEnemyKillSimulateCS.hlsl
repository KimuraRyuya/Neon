...
void main(uint3 DTid : SV_DispatchThreadID, uint Gid : SV_GroupIndex)
{
    ...
    if (DTid.x < aliveCount)
    {
        ...
        if (particle.life > 0)
        {
			...
			// integrate:
            particle.velocity.xz += particle.accel.xz * dt;
            particle.position.xz += particle.velocity.xz * dt;
            
            particle.velocity.y -= particle.accel.y * dt;
            particle.position.y += particle.velocity.y * dt;
         
            if ((0 - particle.position.x) * (0 - particle.position.x) + (0 - particle.position.z) * (0 - particle.position.z) >= (particle.sizeBeginEnd.x + 47) * (particle.sizeBeginEnd.x + 47))
            {
                particle.velocity.xz *= -1;
                particle.accel.xz *= -1;
            }
            
            if (particle.life % particle.maxLife / 4 == 0)
            {
                particle.accel * -1;
            }

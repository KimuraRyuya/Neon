...
void main(uint3 DTid : SV_DispatchThreadID)
{
    ...
    if (DTid.x < emitCount)
    {
        ...
        particle.velocity = float3((rand(seed, uv) - 0.5) * particleParam[constantIndex].xParticleSpeed, 0, (rand(seed, uv) - 0.5) * particleParam[constantIndex].xParticleSpeed);
        particle.accel = float3(particleParam[constantIndex].xParticleSpeed / 20, 0, particleParam[constantIndex].xParticleSpeed / 20);
        ...
        particle.sizeBeginEnd = float2(particleParam[constantIndex].xParticleSize, particleParam[constantIndex].xParticleSize);
        particle.color = 0;

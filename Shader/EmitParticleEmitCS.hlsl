...
void main( uint3 DTid : SV_DispatchThreadID )
{
    ...
    if (DTid.x < emitCount)
    {
        ...
        particle.velocity = float3((rand(seed, uv) - 0.5) * particleParam[constantIndex].xParticleSpeed * nor.x, (rand(seed, uv) + 0.5) * particleParam[constantIndex].xParticleSpeed * nor.y, (rand(seed, uv) - 0.5) * particleParam[constantIndex].xParticleSpeed * nor.z);

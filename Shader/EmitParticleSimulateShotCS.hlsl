...
void main(uint3 DTid : SV_DispatchThreadID, uint Gid : SV_GroupIndex)
{
    ...
    if (DTid.x < aliveCount)
    {
        ...
        if (particle.life > 0)
        {
            ...
            if ((0 - particle.position.x) * (0 - particle.position.x) + (0 - particle.position.z) * (0 - particle.position.z) >= (particle.sizeBeginEnd.x + 98) * (particle.sizeBeginEnd.x + 98))
            {
                 // kill:
                uint deadIndex;

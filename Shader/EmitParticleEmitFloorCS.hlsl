...
void main(uint3 DTid : SV_DispatchThreadID)
{
    ...
    if (DTid.x < emitCount)
    {
        ...
        Particle particle;
        particle.position = float3((rand(seed, uv) - 0.5) * 72.0f, 0.5f, (rand(seed, uv) - 0.5) * 72.0f);
        particle.velocity = float3((rand(seed, uv) - 0.5) * particleParam[constantIndex].xParticleSpeed, (rand(seed, uv) + 0.5) * particleParam[constantIndex].xParticleSpeed, (rand(seed, uv) - 0.5) * particleParam[constantIndex].xParticleSpeed);
        particle.accel = float3((rand(seed, uv) - 0.5) * particleParam[constantIndex].xParticleSpeed, (rand(seed, uv) + 0.5) * particleParam[constantIndex].xParticleSpeed, (rand(seed, uv) - 0.5) * particleParam[constantIndex].xParticleSpeed);
        particle.rotationalVelocity = particleParam[constantIndex].xParticleRotation;

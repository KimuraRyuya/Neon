...
bool FrameWork::initialize()
{
	...
	PAD->InitDInput(hinstance, hwnd);
	PAD->InitDInputMouse(hwnd);
...
void FrameWork::update(float elapsed_time/*Elapsed seconds from last frame*/)
{
	...
	// 隠しコマンドエディット
	{
		...
		if (GetAsyncKeyState(VK_SHIFT) < 0 && GetAsyncKeyState('P') < 0 && !inputTrigger)
		{
			inputTrigger = true;
			if (onGame)
			{
				onGame = false;
				PAD->EnableCursor(true, true);
			}
			else if (!onGame)
			{
				onGame = true;
				PAD->EnableCursor(false, true);
			}
		}
		else if (GetAsyncKeyState(VK_SHIFT) == 0 && GetAsyncKeyState('P') == 0 && inputTrigger)
		{
			inputTrigger = false;
		}
	}
	if (!onGame) elapsedTime = 0.0f;

	pSceneManager->Update(elapsedTime);
